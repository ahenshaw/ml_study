...
\begin{QandA}
    ...
    \begin{answered}
        ...
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.3\textwidth]{data_representation.png}
            \caption{Generic Learning Model - Data Representation}
            \label{fig:data}
        \end{figure}

        \begin{figure}[H]
            \centering
            \includegraphics[width=0.3\textwidth]{knowledge_and_reasoning.png}
            \caption{Generic Learning Model - Knowledge and Reasoning}
            \label{fig:knowledge}
        \end{figure}

        \begin{figure}[H]
            \centering
            \includegraphics[width=0.3\textwidth]{planning_and_action.png}
            \caption{Generic Learning Model - Planning and Action}
            \label{fig:planning}
        \end{figure}
    ...
    \begin{answered}
        ...
        \begin{itemize}
            \item Learner is not told which actions to take
            \item \textit{Trial-and-Error} search
            \item Possibility of delayed reward (sacrifice short-term gains for greater long-term gains)
            \item The need to explore and exploit
            \item Considers the whole problem of a goal-directed agent interacting with an uncertain environment
            \item \textit{Exploration} finds more information about the environment
            \item \textit{Exploitation} exploits known information to maximize reward
            \item It is usually important to explore as well as exploit
        \end{itemize}

        \subsubsection*{Elements of Reinforcement Learning}
        \begin{description}
            \item[Policy:] what to do
            \item[Reward:] what is good
            \item[Value:] what is good because it predicts reward
            \item[Model:] what follows what
        \end{description}
...
\begin{QandA}
    ...
    \begin{answered}
        ...
        \begin{description}
            \item[Code Size] It represents the number of nodes in the middle layer. Smaller size results in more compression.
            \item[Number of Layers] The autoencoder can consist of as many layers as we want.
            \item[Number of nodes per layer] The number of nodes per layer decreases with each subsequent layer of the encoder and increases back in the decoder. The decoder is symmetric to the encoder in terms of the layer structure.
            \item[Loss Function] We either use mean squared error or binary cross-entropy. If the input values are in the range [0, 1] then we typically use cross-entropy, otherwise, we use the mean-squared error.
        \end{description}}
    ...
    \begin{answered}
        \subsubsection*{Types of autoencoders}
        \begin{enumerate}
            \item Stacking
            \item Convolution
            \item Deep
